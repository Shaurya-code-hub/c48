...
function preload(){
  ...
  frameImg = loadImage("images/Bgframe.png");
}

function setup() {
  createCanvas(displayWidth, displayHeight - 120);

  frame = createSprite(displayWidth/2, displayHeight/2);
...
function draw() {
  ...
  if(enemyGroup.isTouching(Indian_soldier)){
    lives = lives - 1;
    enemyGroup.destroyEach();
  }
...
function spawnEnemy(){
  if(frameCount % 80 === 0 ){
    ...
    enemy.debug = true;
    enemy.setCollider("rectangle", -100, 0, 1, 390);
...
function spawnBullet(){
 ...
 //bullet.debug = true;
  bullet.setCollider("rectangle", 0, 0, 460, 360);
  bullet.scale = 0.2;
  bullet.velocityX = 20;
